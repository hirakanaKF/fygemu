    ...
    */
    ...
    const 
        ...
        _acGen = (i, u, D, G) => {
            ...
            const 
                out = {}, req = {}, K = G.Kind, m = u.mActor, t = new Date().getTime(),
                bvip = u.mBVip < t, svip = u.mSVip < t,
                qa = G.L + (bvip ? G.B : 0) + (svip ? G.S : 0), qm = 1 - qa,
                vm = G.Trait, sm = G.Skill, bm = G.Build, sp = G.M
            ;

            for (const i of D) {
                if (i >= m) { continue; }
                const k = K[rngNumber(K.length)], a = new Actor(k), c = a.mCard, r = rngBigInt();
                let 
                    q = rngBeta22() * qm + qa,
        ...
        _ecGen = (i, u, D, G) => {
            const 
                emu = Emu.EmuEquipKind, P = G.Kind, Z = G.Data, K = Object.keys(P), m = u.mEquip,
            ...
            for (const i of D) {
                ...
                const 
                    e = new Emu.Equip(), k = K[rngNumber(K.length)], E = Z[P[k]],
        ...
        _fcGen = (i, u, D, G) => {
            ...
            for (const k in D) {
                ...
                for (const kind in K) {
                    ...
                    d.mStat[attr] = Object.fromEntries(
                        W.slice(0, n).map((k, i) => {
                            const a = A[k];
                            return [k, a.Mul * X[i] + a.Add * Array(l + 1).reduce(n => n + rngBeta22(), 0)];
                        })
                    );
                    ...
                    Emu.Team(
                        lActor.map(r => {
                            ...
                            r.mFruits.forEach(i => {
                                ...
                                if (!e) { return; }
                                const A = e.mStat;
                                for (const k in A) { a[k] = $EquNumCast(Math.min((a[k] ?? 0) + A[k], LF[k])); }
                            });
        ...
        */
        fcFrg = async (s, a, m) => {
            ...
            for (const k of F) {
                const e = E[k], W = Object.keys(e.mStat), w = W[rngNumber(W.length)];
                e.mStat[w] += x * (A.Mul * e.mValue + A.Add * rngBeta22());
