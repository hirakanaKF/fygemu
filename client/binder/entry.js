...
const 
    ...
    // Allocate callback table
    svcLock = (k, T) => svcCall[k] = T,
    ...
    gSetLang = (lang) => {
        ...
        window.gMsgPanelName = msg.PanelName;
        window.gMsgPanelDesc = msg.PanelDesc;
        window.gMsgPanelPrefix = msg.PanelPrefix;
        window.gMsgPanelMiddle = msg.PanelMiddle;
        window.gMsgPanelSuffix = msg.PanelSuffix;
        window.gMsgAttrName = msg.AttrName;
        window.gMsgAttrInfo = msg.AttrInfo;
        ...
        window.gMsgArt1Name = msg.Art1Name;
        window.gMsgArt1Desc = msg.Art1Desc;
        window.gMsgArt2Name = msg.Art2Name;
        window.gMsgArt2Desc = msg.Art2Desc;
        window.gMsgArt3Name = msg.Art3Name;
        window.gMsgArt3Desc = msg.Art3Desc;
        window.gMsgStatusName = msg.StatusName;
        window.gMsgEquipName = msg.EquipName;
        window.gMsgEquipDesc = msg.EquipDesc;
        window.gMsgEquipMyst = msg.EquipMyst;
        ...
        window.gMsgGiftName = msg.GiftName;
        window.gMsgGiftUnit = msg.GiftUnit;
    },
    ...
    uiArt3 = () => {
        ...
        elArts.set = (kind) => {
            elArt1Name.innerHTML = gMsgArt1Name[kind] || kind;
            elArt1Desc.innerHTML = gMsgArt1Desc[kind] || kind;
            elArt2Name.innerHTML = gMsgArt2Name[kind] || kind;
            elArt2Desc.innerHTML = gMsgArt2Desc[kind] || kind;
            elArt3Name.innerHTML = gMsgArt3Name[kind] || kind;
            elArt3Desc.innerHTML = gMsgArt3Desc[kind] || kind;
        };
    ...
    uiItem = (K) => {
        ...
        const
            ...
            num = (n) => {
                ...
                elRoot.style.display = (n > 0) ? "" : "none";
            }
        ;

        // There is an css rule of .btn.fyg_colpzbg.fyg_tc which has higher priority than .btn.fyg_colpzbg.fyg_mp3, and that is why we add text-align to the style.
        elRoot.classList = "btn fyg_colpzbg fyg_mp3 tip-bottom"; elRoot.style = "width:4.7rem;height:4.7rem;text-align:center;";
        ...
        elTitle.append(tnName, " x", tnNum);
        elInner.append(elDesc);
        elDesc.style = "color:#000000; font-weight: normal;";
        icon(); num(N);
    ...
    uiItems = (lyt) => {
        ...
        for (const kind of lyt) {
            const e = uiItem(kind); el.append(e); el[kind] = e;
        }
        return el;
    },
    ...
    uiActor = () => {
        const
            ...
            set = (d) => {
                const {i, K, R, V, L, Q, S, B, F, G, p, str, agi, int, vit, spr, mnd, exp, auras, equip, fruit} = d;

                elRoot.K = K; elRoot.R = R; elRoot.L = L; elRoot.V = V;
                elRoot.Q = Q; elRoot.S = S; elRoot.B = B; elRoot.F = F; elRoot.G = G;
                elRoot.p = p;
                elRoot.str = str; elRoot.agi = agi; elRoot.int = int;
                elRoot.vit = vit; elRoot.spr = spr; elRoot.mnd = mnd;
            ...
            clr = () => {
                set({K: 0, R: 1, V: 0, L: 0, Q: 0, S: 0, B: 0, F: 0, G: 0, p: 0, str: 0, agi: 0, int: 0, vit: 0, spr: 0, mnd: 0, exp: 0, auras: [], equip: [], fruit: []}); open(!1);
        ...
        elRoot.onload = () => {
            ...
            tnName.textContent = gMsgActorName[K] ?? K;
            elDesc.innerHTML = gMsgActorDesc[K] ?? K;
            elTraitL.innerHTML = gMsgPanelPrefix.Trait; tnTraitR.textContent = gMsgPanelSuffix.Trait;
            elSkillL.innerHTML = gMsgPanelPrefix.Skill; tnSkillR.textContent = gMsgPanelSuffix.Skill;
            elBuildL.innerHTML = gMsgPanelPrefix.Build; tnBuildR.textContent = gMsgPanelSuffix.Build;
            elFlairL.innerHTML = gMsgPanelPrefix.Flair; tnFlairR.textContent = gMsgPanelSuffix.Flair;
        };
        elRoot.onreset = icon;
        clr();
    ...
    uiEquip = () => {
        ...
        const
            ...
            set = (d) => {
                const {K, R, L, V, Q, M, S} = d;
                
                elRoot.K = K; elRoot.R = R; elRoot.L = L;
                elRoot.V = V; elRoot.Q = Q; elRoot.M = M;
                elRoot.S = S;

                // Kind
                tnName.textContent = gMsgEquipName[K] ?? K;
                elMyst.innerHTML = gMsgEquipMyst[K] ?? K;
                ...
                elAttrs.innerHTML = Q.map((q, i) => {
                    const [k, v] = S[i], r = (q > 100) ? (q > 130) ? "danger" : (q > 120) ? "warning" : "success" : (q > 80) ? "info" : "primary";
                    return `<p class="fyg_xlxx${r}"><span>${gMsgEquipAttrName[k] || k}</span><span> ${uiNumFix2(v)}</span><span class="pull-right bg-${r}">&nbsp;${q.toFixed()}%&nbsp;</span></p>`
                }).join("");

                // Myst
                elMyst.style.display = M ? "" : "none";
            ...
            clr = () => {
                set({K: 0, R: 1, L: 0, V: 0, Q: [], M: 0, S: []}); open(!1);
                elRoot.checked = !1; elRoot.style.boxShadow = "";
            }
        ;

        elRoot.classList = "btn fyg_colpzbg fyg_mp3 tip-bottom"; elRoot.style = "width:4.7rem;height:4.7rem;";
        ...
        elRoot.show = show;
        elRoot.onload = () => {
            const K = elRoot.K, l = elAttrs.children;
            tnName.textContent = gMsgEquipName[K] ?? K;
    ...
    uiWear = () => {
        const
            ...
            set = (d) => {
                ...
                elAttrs.innerHTML = Q.map((q, i) => {
                    const [k, v] = S[i], r = (q > 100) ? (q > 130) ? "danger" : (q > 120) ? "warning" : "success" : (q > 80) ? "info" : "primary";
                    return `<p class="fyg_xlxx${r}"><span>${gMsgEquipAttrName[k] ?? k}</span><span> ${uiNumCast(v)}</span><span class="pull-right bg-${r}">&nbsp;${q.toFixed()}%&nbsp;</span></p>`
                }).join("");
    ...
    uiShopItem = () => {
        ...
        const
            ...
            eval = () => {
                ...
                for (const k in P) { T.push(n * P[k] + " " + gMsgGiftName[k]); }
            ...
            data = (i, d) => {
                ...
                for (const k in G) {
                    const n = G[k];
                    if (!n) { T.push(gMsgGiftName[k] ?? k); continue; }
                    T.push((gMsgGiftName[k] ?? k) + "×" + G[k] + (gMsgGiftUnit[k] ?? k));
                }
        ...
        elRoot.onload = () => {
            ...
            for (const k in P) { TC.push(n * P[k] + " " + gMsgGiftName[k]); }
            elBuy.innerHTML = TC.join(" + ") || "　";
            for (const k in G) {
                const n = G[k];
                if (!n) { TG.push(gMsgGiftName[k] ?? k); continue; }
                TG.push(gMsgGiftName[k] + "×" + n + gMsgGiftUnit[k]);
            }
    ...
    uiShopBack = () => {
        ...
        const
            ...
            eval = () => {
                const l = gUser[elBuy.i], n = Math.min(elBuy.l - l, uiNumCast(elInput.value)), m = n * n + 2 * l * n, T = [];
                for (const k in P) { T.push(m * P[k] + " " + gMsgGiftName[k]); }
                elBuy.innerHTML = T.join(" + ") || "　";
            ...
            data = (i, p, l) => {
                elName.innerHTML = (gMsgGiftName[i] ?? i); // + "×1" + (gMsgGiftUnit[i] ?? i);
    ...
    const
        bmsgJpt = {
            "stat": (i, m) => `<i class="icon icon-unlink"><b>${gMsgStatusName[i] || i}${gMsgActUIInfo.StatusPrefix}${m}${gMsgActUIInfo.StatusSuffix}</b></i>`,
            "act": (i, m) => `<i class="icon icon-location-arrow"><b>${gMsgActUIInfo[i] || i}${m}</b></i>`,
            "hpot": (i, m) => `<i class="icon icon-bookmark-empty text-success"><b>${gMsgActUIInfo.HpPot}</b></i>`,
            "spot": (i, m) => `<i class="icon icon-bookmark-empty text-success"><b>${gMsgActUIInfo.SdPot}</b></i>`,
            "art1": (i, m) => `<i class="icon icon-location-arrow"><b>${gMsgArt1Name[i]}${gMsgActUIInfo.SkillPrefix}${m}${gMsgActUIInfo.SkillSuffix}</b></i>`,
            "art2": (i, m) => `<i class="icon icon-location-arrow"><b>${gMsgArt2Name[i]}${gMsgActUIInfo.SkillPrefix}${m}${gMsgActUIInfo.SkillSuffix}</b></i>`,
            "art3": (i, m) => `<i class="icon icon-location-arrow"><b>${gMsgArt3Name[i]}${gMsgActUIInfo.SkillPrefix}${m}${gMsgActUIInfo.SkillSuffix}</b></i>`,
            "aura": (i, m) => `<i class="icon icon-bookmark-empty"><b>${gMsgAuraName[i]}${gMsgActUIInfo.AuraPrefix}${m}${gMsgActUIInfo.AuraSuffix}</b></i>`,
            "": (i, m) => ""
        },
        ...
        bmsg = (p) => {
            const S = {};
            p.forEach((t) => {
                const [a, i, m, w] = t, k = bmsgJpt[a](i, w ? gMsgActUIInfo[w][m] : m);
                S[k] = (S[k] || 0n) + 1n;
            });
    ...
    return (data) => {
        ...
        <span class="fyg_f18">${B ? `${gMsgActorName[u.at]}（Lv.${uiNumber(u.lv)} ${gMsgAttrInfo.mIsPVE}）` : `${u.nm}（Lv.${uiNumber(u.lv)} ${gMsgActorName[u.at]}）`}</span><br>
            [${gMsgAttrInfo.mHpMax}:${Math.ceil(u.hp)}] [${gMsgAttrInfo.mSdMax}:${Math.ceil(u.sd)}]<br>
            [${gMsgAttrInfo.mSpd}:${Math.ceil(u.sp)}]<br>
            [${gMsgAttrInfo.mPowP}:${Math.ceil(u.pp)}] [${gMsgAttrInfo.mPowM}:${Math.ceil(u.pm)}]<br>
            [${gMsgAttrInfo.mDefP}:${Math.ceil(u.dp)}] [${gMsgAttrInfo.mDefM}:${Math.ceil(u.dm)}]<br>
            <br>
        </div>
        <div class="col-md-5 fyg_tl">
            <div class="fyg_nw" style="min-height: 6.75rem;">
            ${u.eq.map(
                (e) => `<button type="button" class="btn fyg_colpzbg fyg_mp3" title="${gMsgEquipName[e.k]}" style="width: 47px; height: 47px; background-image: url(${gUsrPath(RES_EQ, e.k, e.r)});" ><br>${e.l}</button>`
            ).join("")}
            </div><br>
            ${Object.keys(gMsgAuraName).map(
        ...
        <span class="fyg_f18">${A ? `${gMsgActorName[u.at]}（${gMsgAttrInfo.mIsPVE} Lv.${u.lv}）` : `${u.nm}（${gMsgActorName[u.at]} Lv.${u.lv}）`}</span><br>
            [${gMsgAttrInfo.mHpMax}:${Math.ceil(u.hp)}] [${gMsgAttrInfo.mSdMax}:${Math.ceil(u.sd)}]<br>
            [${gMsgAttrInfo.mSpd}:${Math.ceil(u.sp)}]<br>
            [${gMsgAttrInfo.mPowP}:${Math.ceil(u.pp)}] [${gMsgAttrInfo.mPowM}:${Math.ceil(u.pm)}]<br>
            [${gMsgAttrInfo.mDefP}:${Math.ceil(u.dp)}] [${gMsgAttrInfo.mDefM}:${Math.ceil(u.dm)}]<br>
    ...
    <div class="col-xs-12"><div class="alert with-icon fyg_tc" style="border:1px #BFBFBF solid;"><h2>${gMsgPanelDesc.PKDraw}</h2></div></div>
    </div>
            `);
            break;
        ...
        case 1n:
            ...
            <div class="row">
    <div class="col-xs-12">&nbsp;</div>
    <div class="col-xs-12"><div class="alert alert-danger with-icon fyg_tc" style="border:1px #EA644A solid;"><i class="icon icon-smile"></i><h2>${(B ? L.map(u => gMsgActorName[u.at]) : L.map(u => u.nm)).join(" & ")}${gMsgPanelDesc.PKWinner}</h2></div></div>
    </div>
            `);
            break;
        ...
        case 2n:
            ...
            <div class="row">
    <div class="col-xs-12">&nbsp;</div>
    <div class="col-xs-12"><div class="alert alert-info with-icon fyg_tc" style="border:1px #03B8CF solid;"><i class="icon icon-frown"></i><h2>${(A ? R.map(u => gMsgActorName[u.at]) : R.map(u => u.nm)).join(" & ")}${gMsgPanelDesc.PKWinner}</h2></div></div>
    </div>
            `);
            break;
        ...
        case 3n:
            ...
            <div class="row">
        <div class="col-xs-12">&nbsp;</div>
        <div class="col-xs-12"><div class="alert with-icon" style="border:1px #BFBFBF solid;"><h4>${gMsgPanelDesc.PKRoundOver}</h4></div></div>
        </div>
            `);
        }
...
global.Engine = {};
global.System = {};
eCursor.classList = "sbox"; zCursor.display = "none";
eSvcRoot.append(eCursor);
